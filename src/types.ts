...
export type FilterName = 'none' | 'warm' | 'vintage' | 'mono';
export type FrameStyle = 'solid'|'blush-grid'|'gingham'|'checker'|'disco'|'cow'|'leopard'|'waves'|'denim'|'floral'|'night'|'sparkle'|'aurora-blue'|'molten-red'|'golden-brush'|'espresso-silk'|'forest-light'|'violet-satin'|'silver-flow'|'ember-chrome'|'dream-flora'|'cosmic-violet'|'ethereal-aura';
...
export interface BoothSettings { frame:string; background:string; gap:number; caption:string; countdown:3|5|10; frameStyle?:FrameStyle; frameEffect?:FrameEffect; photoShape?:PhotoShape; sticker?:string; stickers?:string[]; decorationPlacements?:Record<string,DecorationPlacement>; titleText?:string; footerText?:string; dateText?:string; textColor?:string; fontStyle?:'script'|'editorial'|'clean'; }
