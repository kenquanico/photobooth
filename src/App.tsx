...
export function App(){
 ...
 <Suspense fallback={<Loading/>}>{page==='editor'&&<PhotoEditor layout={layout} photos={photos} settings={settings} onRetake={()=>go('camera')} onLayout={setLayout} onSettings={setSettings}/>} {page==='pair'&&<PairDevice/>}</Suspense>
 <footer><button className="brand" onClick={()=>go('home')}>petal<span>booth</span></button><p>Made for moments worth keeping.</p><small>© {new Date().getFullYear()} Petal Booth · Photos never leave your browser.</small></footer></div>;
}
